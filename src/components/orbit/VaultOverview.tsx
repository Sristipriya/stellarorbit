  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <h3 className="font-display text-sm uppercase tracking-[0.2em] text-[var(--orbit-mute)]">Vault Overview</h3>
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        {stats.map((s) => (
          <div key={s.label} className="rounded-xl border border-[var(--orbit-edge)] bg-black/20 p-3">
            <div className="text-[10px] uppercase tracking-widest text-[var(--orbit-mute)]">{s.label}</div>
            <div className={`mt-1 font-display text-xl ${s.accent ? "text-[var(--orbit-accent)]" : ""}`}>{s.value}</div>
          </div>
        ))}
      </div>
    </motion.div>
