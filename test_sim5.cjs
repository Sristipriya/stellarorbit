...
// Index Vault ID from vaults.ts? Wait, let's check network.ts
const contractId = 'CAXSCGC7FNM3GKPE4BDHYUV2YWC5K3DKRVX5AOR7YP6DIXQNWFYCP2JS';
...
async function test() {
  ...
  try {
    ...
    console.log("INDEX 3 arguments: Prepared successfully");
  } catch (e) {
    console.error("INDEX 3 arguments: Failed to prepare:", e.message || e);
  }
  ...
  try {
    ...
    console.log("INDEX 2 arguments: Prepared successfully");
  } catch (e) {
    console.error("INDEX 2 arguments: Failed to prepare:", e.message || e);
  }
}
